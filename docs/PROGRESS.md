...
- 🟡 **الرحلة الكاملة بلا تدخل يدوي:** كل مكونات الرحلة العشرة منجزة ومختبرة **كوحدات**؛ المتبقي لإغلاق الشرط: **(C5.11) المنسّق** — ربط التدفقات في حلقة رسائل C4 (inbound worker) كمحادثة واحدة متدرجة (موافقات→أسئلة→رفع→تأكيد→مسار→بطاقة→تفعيل) + اختبار رحلة E2E بالحالات السبع، ثم التحقق الحي (يحتاج بوابة C4 الحية + `ANTHROPIC_API_KEY` 👤).

**التالي:** C5.11 المنسّق (ربط + E2E) → إغلاق C5 → C6 (المحرك، يحتاج 👤 مفتاح SerpApi).

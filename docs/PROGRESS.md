...
- **P-B.5 النسخ الاحتياطي 🟡 (15 يوليو):** restic 0.16.4 مثبّت · `ops/backup/` (backup.sh بـ`--stdin` واسم ملف ثابت · restore-test.sh بقاعدة scratch وفحوص سلامة · systemd timers يومي 03:30/شهري · env.example · README) · env سري في `/root/.config/career-backup/backup.env` (0600) بكلمة repo مولّدة (فهد حفظها). **البنية مُختبَرة end-to-end محليًا** (backup→restore→16 جدولًا/13 RLS→PASSED). **يتبقّى:** ربط B2 (فهد ينشئ bucket+key) ثم أول backup+restore **خارجي** فعلي — هذا وحده يُغلق شرط خروج C2 (§13 يشترط «نسخة خارجية»).
